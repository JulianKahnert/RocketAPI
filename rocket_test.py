...
class test_R60V(unittest.TestCase):
    ...
    # known wrong messages (from Checksum.unit.ts) - verification should be: False
    _wrong_message_checksums = [
        # invalid message
        'F00DS0DA33',
    ...
    def test_known_checksums(self):
        for msg in self._checksum_data.keys():
            # expected checksum
            expected = self._checksum_data[msg]
            # calculated checksum
            calc = self.obj.checksum(msg)
    ...
    def test_long_strings(self):
        ...
        # calculate checksum
        calc = self.obj.checksum(long_str)
    ...
    def test_insensitivity_of_byte_order(self):
        ...
        # calculate checksums
        cs = self.obj.checksum(comps)
        cs_shuffled = self.obj.checksum(comps_shuffled)

        self.assertEqual(cs, cs_shuffled)

    def test_wrong_messages(self):
        for wrg_msg in self._wrong_message_checksums:
            self.assertFalse(self.obj.cs_verify(wrg_msg))
